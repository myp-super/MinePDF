...
深色科研风格三栏布局：左侧知识库文件树、中间 PDF 阅读器、右侧信息面板，干净高效，大文件秒开。

![主界面](pic/shot-main.png)

### 分屏阅读

支持网页式标签页与上下/左右分屏：每个屏都是独立的阅读区，拥有自己的标签栏、缩放与书签，互不干扰。

![分屏阅读](pic/shot-split.png)

### 图文笔记

Markdown 笔记支持截图框选插入与 LaTeX 公式，图文并茂边读边记，可导出为 PDF。

![图文笔记](pic/shot-notes.png)

## 核心功能

- **PDF 文件库（3.3.0 多知识库）**：支持多个知识库，每个知识库对应 `Documents/MinePDF/Library/<知识库名>` 真实目录，与软件树严格双向同步（新建/重命名/移动/删除即时反映）；空白区右键可新建知识库、新建子文件夹、导入 PDF，导入前通过目录树选择目标位置；右键直达系统资源管理器
...
- **临时阅读区**：设置里一键设为默认 PDF 应用（写入系统关联 + 打开方式对话框确认），双击任意 PDF 临时预览，不进入知识库
- **全局搜索**：文件名、标签、笔记内容
